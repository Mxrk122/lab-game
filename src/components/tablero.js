...
function Tablero({laberinto, weight, height, updateLabyrinth, win}) {
    ...
    // Mover al personaje
    // ver en que posicion se encuentra el personaje
    ...
    const move = (x, y, plusx, plusy) => {
        ...
        // La p se mueve a la nueva fila, donde reemplazare el espacio vacio por la p, la direcciona la que se mueve el jugador
        const newLine = laberinto[y + plusy];

        //Ahora lo mismo pero horizontalmente
        // Al tratarse de movimiento horizontal solo necesito oldLine para saber en que fila está y mvoerlo desde ahí
        const oldPosition = x
        ...
        if(laberinto[y+plusy][x+plusx] == "-" || laberinto[y+plusy][x+plusx] == "+" || laberinto[y+plusy][x+plusx] == "|"){
          ...
          if (plusx == 0) {
            ...
            oldLine.map((elemento, indice) => {
              // Si se encuentra la p, reemplazarla por u espacio en blanco
              elemento == "p" ? (oldLine[indice] = " ") : null;
            });
    ...
    const pressKey = () => {
        ...
        if(event.key === "s" || event.key === "ArrowDown"){
            // y -> va primero, ya que es la fila. || Luego va x ya que es la posicion de la fila donde está
            move(x, y, 0, 1)
        }
